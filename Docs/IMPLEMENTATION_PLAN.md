...
| **Infamy-scaled danger** ✅ | Kenshi | `PartyState.bounty`, `TravelDirector.RollWantedPatrol` | **Done.** A `bounty` makes `WantedInVoidovia` real: fall to ≤ −20 Voidovia relation (or pick the outlaw origin) and a bounty lands. While wanted, **Voidovia patrols hunt the roads** (chance climbs with the bounty) — a new encounter fielding real Void troops, so a win *captures Void soldiers*. Fight (name darkens), bribe past, or flee; lose and a fine chips at the bounty. Clear your name by **paying it off at a Voidovia settlement**. Being `Infamous` also raises general road danger. HUD shows "WANTED {g}", persisted (save v6). *Next: towns could bar/toll a wanted outlaw at the gate.* |
| **Caravans & trade routes** ◐ | M&B | `TravelDirector` (Trader), `WorldMapUI` | **Done (encounter side).** The Trader encounter is now a **merchant caravan** with hired guards, carrying rolled cargo (coin + `grain_trade`). Three choices: **Trade** (pay 25g for trade goods to resell in town — safe arbitrage, +Traders relation), **Raid** (fight the guards; win → loot the coin + goods + capture Trader guards, but −8 Traders and −5 Voidovia, pushing you toward Wanted), or pass. *Next: caravans as economic world-parties routing between prosperous markets, and hooking the existing escort quest to them.* |
| **Notables & recruitment** ✅ | M&B | `SettlementState.notableRelation`, `MarketService` | **Done.** Each settlement has a notable whose **regard (0–100)** gates recruiting: it scales recruit **stock** (0.4×→1.4×, stacked with prosperity), gives a **price discount** (up to 30%), and past regard 60 the settlement **offers its culture's next tier** instead of the base troop. **Quests for the settlement raise regard** (+12); a **failed/expired quest drops it** (−15). Shown in the settlement inspect ("Notable regard: 72/100 — trusts you") and the recruit button. Persisted (save v7). Recruiting is now a relationship, not a flat transaction. |
