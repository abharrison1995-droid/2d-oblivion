...
namespace Voidovia
{
    ...
    public class SettlementMenuUI : MonoBehaviour
    {
        ...
        void Ensure()
        {
            ...
            var root = UiFactory.Panel(_canvas.transform, "Root", Vector2.zero, Vector2.one, new Color(0.08f, 0.09f, 0.1f, 0.98f));

            UiFactory.Portrait(root, "Image", new Vector2(0.1f, 0.62f), new Vector2(0.9f, 0.86f), "settlement_placeholder");

            _description = UiFactory.Label(root, "Description", "", 23, TextAnchor.MiddleCenter, new Color(0.72f, 0.72f, 0.68f));
            Stretch(_description, 0.05f, 0.55f, 0.95f, 0.62f);

            _title = UiFactory.Label(root, "Title", "", 34, TextAnchor.MiddleCenter, new Color(0.93f, 0.86f, 0.7f));
            Stretch(_title, 0.05f, 0.48f, 0.95f, 0.55f);

            _subtitle = UiFactory.Label(root, "Subtitle", "", 20, TextAnchor.MiddleCenter, new Color(0.75f, 0.75f, 0.72f));
        ...
        void Rebuild(string nodeId)
        {
            ...
            if (actions.Count == 0)
            {
                var empty = UiFactory.Label(_actionsRoot, "Empty", "Nothing to do here yet.", 20, TextAnchor.MiddleCenter, new Color(0.6f, 0.6f, 0.6f));
