...
namespace Voidovia
{
    ...
    public class WorldMapUI : MonoBehaviour
    {
        ...
        void BuildChrome()
        {
            ...
            _inspectTitle = UiFactory.Label(content, "InspectTitle", "Select a place", 26, TextAnchor.UpperLeft, new Color(0.93f, 0.86f, 0.7f));
            Stretch(_inspectTitle, 0f, 0.87f, 0.58f, 1f);
            // Taller body so the settlement lines (type, route, services, prosperity, wars) fit without
            // spilling down over the log below.
            _inspectBody = UiFactory.Label(content, "InspectBody", "", 18, TextAnchor.UpperLeft, new Color(0.8f, 0.8f, 0.78f));
            Stretch(_inspectBody, 0f, 0.5f, 0.58f, 0.86f);
            ...
            // Left-align every menu button's caption so the leading glyphs (→ ◆ ■ ! ? § ● ○) line up
            // down each column instead of floating centred at different offsets.
            foreach (Transform child in content)
            {
                var menuBtn = child.GetComponent<Button>();
                if (menuBtn == null) continue;
                var lbl = menuBtn.GetComponentInChildren<Text>(true);
                if (lbl != null) lbl.alignment = TextAnchor.MiddleLeft;
            }

            var logPanel = UiFactory.Panel(content, "LogPanel", new Vector2(0.02f, 0.02f), new Vector2(0.58f, 0.47f), new Color(0.05f, 0.06f, 0.08f, 0.5f));
            ...
            _logText = UiFactory.Label(logPanel, "Log", "", 21, TextAnchor.UpperLeft, new Color(0.75f, 0.8f, 0.78f));
        ...
        void BuildEncounterPanel(Transform root)
        {
            ...
            _encounterTitle = UiFactory.Label(panel, "T", "", 26, TextAnchor.UpperLeft, new Color(0.95f, 0.85f, 0.6f));
        ...
        void BuildTravelConfirmPanel()
        {
            ...
            _travelConfirmTitle = UiFactory.Label(panel, "Title", "", 27, TextAnchor.UpperCenter, new Color(0.95f, 0.88f, 0.7f));
            Stretch(_travelConfirmTitle, 0.05f, 0.6f, 0.95f, 0.95f);
            _travelConfirmBody = UiFactory.Label(panel, "Body", "", 20, TextAnchor.UpperCenter, new Color(0.82f, 0.82f, 0.8f));
