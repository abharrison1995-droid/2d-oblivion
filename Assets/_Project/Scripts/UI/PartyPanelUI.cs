...
namespace Voidovia
{
    ...
    public class PartyPanelUI : MonoBehaviour
    {
        ...
        void Ensure()
        {
            ...
            _list = UiFactory.Panel(root, "List", new Vector2(0.05f, 0.17f), new Vector2(0.95f, 0.72f), new Color(0, 0, 0, 0.18f));

            // Two-column tab grid so the buttons read cleanly instead of a cramped 6-across strip.
            // Rows (top→bottom): Hero|Troops, Pouch|Captives, Comp|Close.
            const float colLx0 = 0.05f, colLx1 = 0.49f, colRx0 = 0.51f, colRx1 = 0.95f;
            float BtnTop(int row) => 0.16f - row * 0.054f;
            float BtnBottom(int row) => BtnTop(row) - 0.046f;

            TabButton(root, "Hero", "Hero", colLx0, colLx1, BtnTop(0), BtnBottom(0), () => SelectTab(ref _heroMode));
            TabButton(root, "Troops", "Troops", colRx0, colRx1, BtnTop(0), BtnBottom(0), () => SelectTab(ref _troopsMode));
            TabButton(root, "Pouch", "Pouch", colLx0, colLx1, BtnTop(1), BtnBottom(1), () => SelectTab(ref _pouchMode));
            TabButton(root, "Captives", "Captives", colRx0, colRx1, BtnTop(1), BtnBottom(1), () => SelectTab(ref _prisonersMode));
            TabButton(root, "Comp", "Comp", colLx0, colLx1, BtnTop(2), BtnBottom(2), () => SelectTab(ref _companionsMode));
            UiFactory.Button(root, "Close", "Close", new Vector2(colRx0, BtnBottom(2)), new Vector2(colRx1, BtnTop(2)), () =>
            {
        ...
        Button TabButton(Transform parent, string name, string caption, float xMin, float xMax, float top, float bottom, System.Action onClick) =>
            UiFactory.Button(parent, name, caption, new Vector2(xMin, bottom), new Vector2(xMax, top), onClick);

        /// <summary>Switch to a single tab: clears every mode, then sets the chosen one (toggling it off if
        /// it was already active, which drops back to the default inventory view).</summary>
        void SelectTab(ref bool mode)
        {
            var newValue = !mode;
            _heroMode = _troopsMode = _pouchMode = _prisonersMode = _companionsMode = false;
            mode = newValue;
            Rebuild();
        }
        ...
        {
            var hero = g.Hero;
            PlaceLabel($"XP: {hero.xp}/{hero.XpForNextLevel()} to level {hero.level + 1}", ref y, size: 23);
            PlaceLabel($"Unspent skill points: {hero.unspentSkillPoints}", ref y, size: 23);
            PlaceLabel($"Warband Experience rank: {hero.warbandExperienceRank} (+{hero.WarbandExperienceDailyGain()}/day)", ref y, size: 23);
            PlaceLabel($"Party: {g.PartyHeadcount}/{g.MaxPartySize} (Leadership sets the cap; +{GameConstants.PartySizePerLeadership} men per point)", ref y, size: 23);

            // Stat buttons two-per-line so all six fit without spilling into the tab row below.
            StatPair(hero, "Combat", hero.combat, () => hero.combat++, "Leadership", hero.leadership, () => hero.leadership++, ref y);
            StatPair(hero, "Tactics", hero.tactics, () => hero.tactics++, "Trade", hero.trade, () => hero.trade++, ref y);
            StatPair(hero, "Scouting", hero.scouting, () => hero.scouting++, "Warband rank", hero.warbandExperienceRank, () => hero.warbandExperienceRank++, ref y);
        }

        void StatPair(CharacterStats hero, string labelA, int valueA, System.Action applyA,
            string labelB, int valueB, System.Action applyB, ref float y)
        {
            var (top, bottom) = UiFactory.NextRow(ref y, 0.1f, 0.02f);
            StatButton(hero, labelA, valueA, applyA, 0f, 0.49f, top, bottom);
            StatButton(hero, labelB, valueB, applyB, 0.51f, 1f, top, bottom);
        }

        void StatButton(CharacterStats hero, string label, int value, System.Action apply, float xMin, float xMax, float top, float bottom)
        {
            var btn = UiFactory.Button(_list, label + "_stat", $"{label}: {value}  (+1)", new Vector2(xMin, bottom), new Vector2(xMax, top), () =>
            {
        ...
        void PlaceLabel(string text, ref float y, float top = -1, float bottom = -1, int size = 18)
        {
            if (top < 0)
            {
                top = y;
            ...
            var lbl = UiFactory.Label(_list, "L" + y, text, size, TextAnchor.MiddleLeft, Color.white);
